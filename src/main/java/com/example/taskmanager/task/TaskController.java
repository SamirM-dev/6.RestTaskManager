...
@RequestMapping("/api/v1") public class TaskController {
    ...
    @PutMapping("/tasks/{id}")
    public ResponseEntity<TaskResponse> update(@PathVariable Long id,@RequestBody TaskUpdateRequest request){
        return ResponseEntity.ok(taskService.update(id,request));
    }

    @PatchMapping("/tasks/{id}/status")
    public ResponseEntity<TaskResponse> statusUpdate(@PathVariable Long id,@RequestBody TaskStatusUpdateRequest request){
    ...
    @GetMapping("/tasks")
    public ResponseEntity<List<TaskResponse>> getAll(@RequestParam String status, @RequestParam String priority, Pageable pageable){
